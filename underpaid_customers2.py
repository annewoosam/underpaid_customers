...
def print_payment_status(payment_data_filename):
    ...
    for line in payment_data:
        ...
        order = line.split('|')# because the pipe is the separator. In many cases it will be a tab or a comma

        # Get customer's full name
        full_name = order[1] #second column in line

        # Split `customer_name` by space (' ') to get
        # a list of [first_name, last_name].
        #
        # Then, assign first name (at index 0) to `customer_first`
        first_name = full_name.split(" ")[0] #tells to split and take first item in split

        # Get no. of melons in the order and amount customer paid
        melons_qty = int(order[2]) 
        amt_paid = float(order[3])# float for currency
        difference = float(order[3])-float(order[2])
        ...
        print(f"{full_name} paid ${amt_paid:.2f}, expected",
              f"${expected_price:.2f}", "for a difference of", f"${difference:.2f}")
        # $ .2f  for currec=ncg and f for avoiding strip space.
